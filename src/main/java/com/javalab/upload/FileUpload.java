...
/**
	...
	multipart/form-data라는 형태로 인코딩 되어 전송되는데, 이를 다루려면
	많은 번거로움이 있습니다. 따라서 파일 업로드를 지원해주는 Apache의 오픈소스
	파일 업/다운 라이브러리를 사용해야함.
...
public class FileUpload extends HttpServlet {
	...
	private void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 	...
		 	되기 때문이다. 임시파일로 저장되었던 파일은 이우해 FileItem#write()
		...
		try {
			/*
			 	...
			 	데이터에 대한 객체입니다. FileTtem#isFormField() 메소드의 리턴값이
			 	true이면 text 일반 입력 데이터이며, false이면 파일 데이터 입니다.
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (fileItem.isFormField()) {	// text 같은 일반 입력 데이터(skip)
					...
					if (fileItem.getSize() > 0 ) {
						...
						// 파일이 업로드될 곳의 풀 경로를 갖는 File 객체 생성(경로 + 파일명)
